"""Separate training-only paired-action calibration campaign; no diagnostic trajectories."""
...
BASE=parent.ROOT/'runs/icra_paired_calibration_20260908'
...
def main():
    ...
    all_tasks=parent.build_tasks('id',32,selection_seed=62112007,waypoint_base=63112007)
    assert len(all_tasks)==32
    diagnostic=set()
    for reg in ['mass_1p4','lag_3']:
        old=parent.ROOT/'runs/icra_vertical_authority_20260908'/('config001' if reg=='mass_1p4' else 'config005')
        for e in json.loads((old/'results.json').read_text())['episodes']:diagnostic.add((e['scene'],e['wind'],e['wp_seed']))
    manifest=[]
    for i,t in enumerate(all_tasks):
        assert t['scene'].seed<4 and (t['scene_id'],t['wind_id'],t['wp_seed']) not in diagnostic
        manifest.append(dict(parent_id=i,split='fit' if i<24 else 'validation',scene=t['scene_id'],wind=t['wind_id'],wp_seed=t['wp_seed']))
    (dest/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
    for ti in range(4):
        tier=f'batch{ti}'
        tasks=all_tasks[ti*8:(ti+1)*8]
        ...
        def hook(sim,ctrl,buf,thrust,active,alive,centers,half,mask,wps,zref,step):
            ...
            metadata.append(dict(tier=tier,step=step,activation_step=step+4,tasks=len(tasks),prefix_alive=int(alive.sum()),duplicate_center_exact=True,parent_unmodified=True))
        out,arrays=parent.rollout(tasks,a.regime,'feedback','cuda',0,None,64112007+1000*ti,snapshot_hook=hook)
        calls=0
        replay,replay_arrays=parent.rollout(tasks,a.regime,'feedback','cuda',0,None,64112007+1000*ti)
        for key in arrays:np.testing.assert_array_equal(arrays[key],replay_arrays[key])
        (dest/f'{tier}_parent.json').write_text(json.dumps(out,indent=2)+'\n')
        metadata[-1]['full_parent_replay_verified']=True
    (dest/'acceptance.json').write_text(json.dumps(dict(regime=a.regime,snapshots=metadata,stage='separate development calibration collection; no model fits'),indent=2)+'\n')
    print('PAIRED CALIBRATION COMPLETE',a.regime,flush=True)
