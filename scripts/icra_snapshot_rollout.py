"""Snapshot-instrumented copy of the accepted matrix rollout; original retained."""
...
@torch.no_grad()
def rollout(tasks,regime,method,device,model_seed,subset,seed,duration_s=12.,snapshot_hook=None):
    ...
    for step in range(n_steps):
        ...
        if step%REC_EVERY==0:
            ...
            if snapshot_hook is not None and step in [600,1200,1800]:
                snapshot_hook(sim,ctrl,buf,thrust,active,alive,centers,half,mask,wps_np,z_ref,step)
            if t<2:
                for i in range(N):
