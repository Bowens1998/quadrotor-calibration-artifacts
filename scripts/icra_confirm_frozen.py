"""Fresh confirmation ridge calibration; equations unchanged from development."""
import argparse,hashlib,json,time
from pathlib import Path
import numpy as np
import torch
from _common import ROOT
from icra_adaptation_cache import INPUT_KEYS
from icra_confirm_common import BASE,FIXTURE
...
def main():
    ...
    ap.add_argument('--config',type=int,required=True);ap.add_argument('--device',default='cpu');ap.add_argument('--attempt',default='frozen_v1');ap.add_argument('--campaign',type=int,required=True);ap.add_argument('--fixture',action='store_true')
    a=ap.parse_args();torch.set_num_threads(4);started=time.time();recipe,seed=CONFIGS[a.config]
    root=FIXTURE if a.fixture else BASE
    if not a.fixture:assert json.loads((BASE/'LOCK.json').read_text())['formal_collection_authorized']
    cache=root/f'campaign{a.campaign}'/a.regime/'cache';meta=json.loads((cache/'metadata.json').read_text())
    assert meta['fixture']==a.fixture and meta['campaign']==a.campaign
    assert set(meta['subsets'])=={'901'} and set(meta['subsets']['901'])=={'32','64'}
    dest=root/f'campaign{a.campaign}'/a.regime/a.attempt/f'{recipe}_seed{seed}';dest.mkdir(parents=True,exist_ok=False)
    ...
    assert len(meta['calibration_episodes'])==64 and len(meta['branch_tasks'])==736
    ...
    result=dict(regime=a.regime,recipe=recipe,model_seed=seed,stage='excluded fixture' if a.fixture else 'confirmation',campaign=a.campaign,fixture=a.fixture,feature_dim=x.shape[1],branch_tasks=meta['branch_tasks'],records=records,physical_records=physical_records,wall_s=time.time()-started)
