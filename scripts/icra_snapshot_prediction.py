...
BASE=ROOT/'runs/icra_snapshot_prediction_20260908'
CONFIGS=[(m,0,s) for m in ['scalar','axis','physics_features','raw'] for s in range(901,906)]+[('supervised',seed,s) for s in range(901,906) for seed in range(5)]
...
def main():
    ...
    assert len(inputs)==9
    for index,(method,seed,subset) in enumerate(CONFIGS):
        p=MatrixPredictor(method,arg.regime,'cuda',seed,subset);rows=[]
        for name,a in inputs.items():
            ...
            for j in range(12):
                ...
                rows.append(row)
        (dest/f'config{index:03d}.json').write_text(json.dumps(dict(regime=arg.regime,method=method,model_seed=seed,subset=subset,predictor_provenance=p.hashes,rows=rows),indent=2)+'\n')
    print('SNAPSHOT PREDICTION COMPLETE',arg.regime,len(CONFIGS),flush=True)
